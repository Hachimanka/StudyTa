...
export default function FlashCardMode() {
  ...
  const recordStudySession = async (topic, durationMinutes) => {
    try {
      ...
      await fetch(`${apiBase}/api/analytics/session`, {
        method: 'POST',
  ...
  const handleCardClick = () => {
    ...
    if (startTime) {
      ...
      setStartTime(Date.now()); // Reset for next interaction
    }
  };
          ...
          <aside className="lg:col-span-3">
            <div className={`border border-[#6F422B] ${darkMode ? 'bg-[#2e2119]' : 'bg-[#F3DAC6]'} rounded-xl p-6 shadow-lg h-[500px]`}>
              ...
              <div className="space-y-4">
                <button onClick={() => maybeConfirmSwitch('flashcards')} className={`w-full flex items-center space-x-4 p-3 rounded-lg border-2 ${darkMode ? 'bg-[#2e2119] border-[#8D5A3F]' : 'bg-[#F3DAC6] border-[#6F422B]'} shadow-lg`}>
