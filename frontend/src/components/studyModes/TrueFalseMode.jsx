...
export default function TrueFalseMode() {
  ...
  // Save current quiz to SavedStudySet collection (no scores)
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/studymode/saved-sets/save`, {
        method: 'POST',
          ...
          <aside className="lg:col-span-3">
            <div className={`border border-[#6F422B] ${darkMode ? 'bg-[#2e2119]' : 'bg-[#F3DAC6]'} rounded-xl p-6 shadow-lg h-[500px]`}>
              ...
              <div className="space-y-4">
                ...
                <button disabled={fromSavedSet} onClick={() => !fromSavedSet && maybeConfirmSwitch('trueFalse')} className={`w-full flex items-center space-x-4 p-3 rounded-lg border-2 ${darkMode ? 'bg-[#2e2119] border-[#8D5A3F]' : 'bg-[#F3DAC6] border-[#6F422B]'} shadow-lg ${fromSavedSet ? 'opacity-50 cursor-not-allowed' : ''}`}>
