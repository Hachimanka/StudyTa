...
export default function TrueFalseMode() {
  ...
  if ((!questions || questions.length === 0) && typeof window !== 'undefined') {
    try {
      ...
      if (sessRaw) {
        ...
        // also allow deriving a title from session.sourceText below via existing logic
      }
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/studymode/save-quiz', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        try {
          ...
          const entry = {
            id: Date.now(),
            title: sess?.title || title || 'Study Session',
            date: new Date().toLocaleDateString(),
            mode: sess?.mode || 'trueFalse',
            questions: sess?.questions || questions,
            sourceText: sess?.sourceText || ''
          };
          const rawSaved = localStorage.getItem('studyta_saved_sets');
          const arr = rawSaved ? JSON.parse(rawSaved) : [];
          // avoid duplicates by title+mode
        ...
        } catch (e) { console.warn('Failed to persist local saved-set after save', e); }
  ...
  const checkIfSaved = () => {
    try {
      ...
      const found = arr.find(a => a.title === (sess.title || title) && a.mode === (sess.mode || 'trueFalse'));
  ...
  // Initialize title from navigation state if available (or fallback to session)
  useEffect(() => {
    if (state?.title) {
      setTitle(state.title);
      setTempTitle(state.title);
    } else if ((state?.sourceText || (typeof window !== 'undefined' && (() => { try { const s=sessionStorage.getItem('studyta_session'); return s ? JSON.parse(s).sourceText : null; } catch(e){return null;} })())) && !editingTitle) {
      // derive short title from sourceText (first sentence or first 6 words)
      const s = String(state?.sourceText || (typeof window !== 'undefined' ? (() => { try { const s=sessionStorage.getItem('studyta_session'); return s ? JSON.parse(s).sourceText : ''; } catch(e){return ''; } })() : '')).trim();
      if (s) {
        const firstSentence = s.split(/(?<=[.?!])\s+/)[0] || s;
        const words = firstSentence.split(/\s+/).slice(0,6).join(' ');
        setTitle(words.length ? words : 'Study Session');
        setTempTitle(words.length ? words : 'Study Session');
      }
    }
  }, [state, editingTitle]);
  ...
  const handleAnswer = (val) => {
    ...
    // advance after short delay, or finish if last
    setTimeout(() => {
      setSelected(null);
  ...
  const generateAndNavigate = async (targetMode) => {
    try {
        ...
        if (resp.ok) {
          ...
          if (Array.isArray(j.questions) && j.questions.length) {
            ...
            switch (targetMode) {
              ...
              default: navigate('/TrueFalseMode', { state: { questions: j.questions, sourceText, title: finalTitle } }); break;
            }
            return;
          }
        }
      ...
      // Fallback: small heuristic generator (use simple sentence splitting)
      ...
      } else if (targetMode === 'flashcards') {
        generated = sentences.map(s => {
          const words = s.replace(/[^\w\s]/g,'').split(/\s+/).filter(Boolean);
          const term = words.find(w => w.length > 4) || words[0] || 'Term';
          return { front: s.slice(0, 70), back: term };
        });
      } else {
        generated = sentences.map(s => ({ statement: s, answer: true }));
      }

      // persist and navigate with fallback
      ...
      switch (targetMode) {
        ...
        default: navigate('/TrueFalseMode', { state: { questions: generated, sourceText, title: finalTitle } }); break;
      }
    } catch (err) {
  ...
  const maybeConfirmSwitch = (targetMode) => {
    if (targetMode === (state?.mode || (sessionStorage.getItem('studyta_session') ? JSON.parse(sessionStorage.getItem('studyta_session')).mode : null))) {
      // switching to same mode — just generate/navigate
      generateAndNavigate(targetMode);
      return;
    }
    // If there is a session but it's already saved locally, skip asking to save
    ...
    <div className={`flex min-h-screen transition-colors duration-500 ${darkMode ? 'bg-[#1f1b16] text-[#f5e9df]' : 'bg-[#F2D9C7] text-[#4A2C1E]'}`}>
      ...
      <main className="p-12 flex-1 ml-20 md:ml-30 mr-7.5 transition-all duration-300">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={handleSave} disabled={saving || savedAlready} className={`flex items-center justify-center space-x-3 px-4 py-2 rounded-full text-sm md:px-8 md:py-3 md:text-lg md:w-48 w-full ${savedAlready ? 'bg-gray-400 cursor-not-allowed text-white' : (darkMode ? 'bg-[#3a2a20] text-white' : 'bg-[#8D5A3F] text-white')}`}>
            ...
            <span>{savedAlready ? 'Saved' : (saving ? 'Saving...' : 'Save')}</span>
          </button>
          {saveMessage && <div className={`ml-3 text-sm ${darkMode ? 'text-gray-200' : 'text-[#6F422B]'}`}>{saveMessage}</div>}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-10">
          <aside className="lg:col-span-3">
            <div className={`border border-[#6F422B] ${darkMode ? 'bg-[#2e2119]' : 'bg-[#F3DAC6]'} rounded-xl p-6 shadow-lg h-[500px]`}>
              ...
              <div className="space-y-4">
                ...
                <button onClick={() => maybeConfirmSwitch('trueFalse')} className={`w-full flex items-center space-x-4 p-3 rounded-lg border-2 ${darkMode ? 'bg-[#2e2119] border-[#8D5A3F]' : 'bg-[#F3DAC6] border-[#6F422B]'} shadow-lg`}>
                ...
                <button onClick={() => maybeConfirmSwitch('multipleChoice')} className={`w-full flex items-center space-x-4 p-3 rounded-lg border ${darkMode ? 'bg-[#2e2119] border-gray-600' : 'bg-[#F3DAC6] border-[#6F422B]'} shadow-sm`}>
                  <svg className="w-8 h-8" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                    <g fill="none">
          ...
          <section className="lg:col-span-7">
            <div className={`${darkMode ? 'bg-[#2e2119]' : 'bg-white'} rounded-xl p-6 shadow-lg h-[500px] flex flex-col relative`}>
              <div className="flex-1 flex items-center justify-center">
                {!finished ? (
                  <div>
                    <h2 className={`text-2xl font-bold text-center ${darkMode ? 'text-white' : 'text-[#6F422B]'}`}>
                      {questions.length ? questions[qIndex]?.statement : 'No questions available — create some materials first.'}
                    </h2>
                  </div>
                ) : (
                  // Finished view
              ...
              {/* Bottom Controls: placed inside the card, slightly above the bottom */}
              {!finished && (
                <div className="absolute left-0 right-0 bottom-16 flex items-center justify-center z-10">
                  <div className="max-w-4xl w-full px-6">
                    <div className="grid grid-cols-3 gap-8 items-center">
                      <button
                        onClick={() => handleAnswer(true)}
                        className={`py-3 rounded-full font-bold text-lg border-3 ${
                          darkMode
                            ? 'border-gray-600 text-white hover:bg-[#3a2a20] hover:border-transparent'
                            : 'border-[#8D5A3F] text-[#6F422B] hover:bg-gray-50 hover:border-transparent'
                        } transition-all duration-200 ${selected === true ? 'ring-2 ring-green-400' : ''}`}
                      >
                        True
                      </button>

                      <div className={`text-center text-lg font-semibold ${darkMode ? 'text-gray-300' : 'text-[#8D5A3F]'}`}>
                      ...
                      <button
                        ...
                        className={`py-3 rounded-full font-bold text-lg border-3 ${
                          darkMode
                            ? 'border-gray-600 text-white hover:bg-[#3a2a20] hover:border-transparent'
                            : 'border-[#8D5A3F] text-[#6F422B] hover:bg-gray-50 hover:border-transparent'
                        } transition-all duration-200 ${selected === false ? 'ring-2 ring-red-400' : ''}`}
                      >
                        False
                      </button>
                    </div>
                    ...
                    <div className="mt-4 text-center">
                      {showFeedback && (
                        <div className={`inline-block px-4 py-2 rounded ${selected === questions[qIndex]?.answer ? 'bg-green-600 text-white' : 'bg-red-600 text-white'}`}>
                          {selected === questions[qIndex]?.answer ? 'Correct' : `Incorrect — Correct: ${questions[qIndex]?.answer ? 'True' : 'False'}`}
                        </div>
                      )}
                    </div>
                  </div>
              ...
              {/* Progress Bar placed inside card bottom */}
              <div className="absolute left-0 right-0 bottom-4 flex flex-col items-center">
                <div className="w-3/4">
                  <div className={`w-full rounded-full overflow-hidden transition-colors duration-200 ${darkMode ? 'border border-[#6F422B] bg-transparent' : 'border border-[#6F422B] bg-[#FDEDE4]'}`} style={{ height: 14 }}>
                    <div className="h-full bg-[#6F422B] rounded-full transition-all duration-300" style={{ width: `${questions.length ? Math.round(((qIndex + (finished ? 1 : 0)) / questions.length) * 100) : 0}%` }} />
                  </div>
                </div>
                <div className={`mt-3 text-sm font-semibold ${darkMode ? 'text-gray-300' : 'text-[#6F422B]'}`}>Progress</div>
              </div>
